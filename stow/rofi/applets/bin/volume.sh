...
# Volume Info
speaker="$(wpctl get-volume @DEFAULT_AUDIO_SINK@ | awk '{print int($2 * 100)}')%"
...
# Speaker Info
if wpctl get-mute @DEFAULT_AUDIO_SINK@ | grep -q "false"; then
    active="-a 1"
    stext='Desmutar'
    sicon=''
else
...
# Microphone Info
if wpctl get-mute @DEFAULT_AUDIO_SOURCE@ | grep -q "false"; then
